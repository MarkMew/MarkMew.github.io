#!/usr/bin/env ruby
#
# Generate multi-language sitemap after Jekyll build completes
...
Jekyll::Hooks.register :site, :post_write do |site|
  ...
  posts_base = File.join(site.source, '_posts')
    ...
    Dir.glob(File.join(posts_base, '*')).each do |lang_dir|
      ...
      Dir.glob(File.join(lang_dir, '*.md')).sort.reverse.each do |post_path|
        begin
          ...
          if lang_code != site.config['default_lang']
            case lang_code
            when 'en'
  ...
  Jekyll.logger.info("✓ Sitemap: Generated with #{all_posts.length} posts across #{langs}")
end
